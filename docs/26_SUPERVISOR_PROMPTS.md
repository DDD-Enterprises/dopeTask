...
Runtime note: the low-level `tp exec` plane currently supports `gemini`, `codex`, and `claude_code`. Route/orchestrate remains a separate runner plane; do not assume parity between those surfaces.
